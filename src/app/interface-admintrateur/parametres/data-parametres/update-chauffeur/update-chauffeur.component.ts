...
export class UpdateChauffeurComponent implements OnInit {
  ...
  getData() {
    this.route.queryParams.subscribe(params => {
      ...
      this.chauffeurId = +params['chauffeur_id'];
    });
    ...
    setTimeout(() => {
      this._dataService.PersonnalUpdateChauffeur(this.chauffeurId, formData).subscribe(
        (response) => {
          ...
          const userType = this._dataService.getUserType();
          if (userType === 'partenaire') {
            this.router.navigate(['/app/partenaire/mes-chauffeurs'], { queryParams: { entrepriseId: this.entrepriseId } });
          } else {
            this.router.navigate(['/app/parametres/chauffeurs', this.entrepriseId, this.type_entreprise]);
          }
        },
        (error) => {
          this.loading = false;
