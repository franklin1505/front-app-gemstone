...
@Component({
  selector: 'app-module-resa-client-sc',
  templateUrl: './module-resa-client-sc.component.html',
  styleUrls: ['./module-resa-client-sc.component.css']
})
export class ModuleResaClientScComponent implements OnInit {
  ...
  sectionValider(nextCallback: any) {
    ...
    this.validerInformations((validationReussie: boolean) => {
      if (validationReussie) {
        // Passer à la section suivante
        this.valider = true;
        this.sectionValiderVisible = true;
        this.nextStep(nextCallback); // Aller à l'étape suivante
      } else {
        console.log("Veuillez renseigner tous les champs obligatoires.");
  ...
  validerInformations(callback: (result: boolean) => void) {
    if (this.clientType === "ancien") {
      // Valider les informations pour un client ancien
      this.validerClientAncien(callback);
    } else if (this.clientType === "nouveau") {
      // Valider les informations pour un nouveau client
      this.validerNouveauClient(callback);
    } else {
      callback(false); // Par défaut, la validation échoue
    }
  }

  validerClientAncien(callback: (result: boolean) => void) {
    const emailOrPhone = this.loginForm.get("email_ou_telephone")?.value;
    const passCode = this.loginForm.get("passCode")?.value;
    // Vérifiez si les informations de connexion sont renseignées
    if (!emailOrPhone || !passCode) {
      console.error("Les informations de connexion ne sont pas renseignées");
      this.errorMessage = "Les informations de connexion ne sont pas renseignées"
      this.showError(this.errorMessage);
      callback(false);
      return;
    }
    // Authentifiez l'utilisateur
    this.sontChampsRenseignes(emailOrPhone, passCode, (result: boolean) => {
      callback(result);
    });
  }

  private sontChampsRenseignes(emailOrPhone: string, passCode: string, callback: (result: boolean) => void) {
    this._estimationService.login({ email_ou_telephone: emailOrPhone, passCode }).subscribe(
      (response: any) => {
        // Si la connexion est réussie
        console.log("Authentification réussie", response);
        // Utiliser emailOrPhone pour récupérer les informations du client
        this.recupererInformationsUtilisateur(emailOrPhone, (clientFound: boolean) => {
          if (clientFound) {
            callback(true);
          } else {
            this.errorMessage = "Aucun client trouvé avec cet email ou numéro de téléphone"
            this.showError(this.errorMessage);
            callback(false);
          }
        });
  ...
  private recupererInformationsUtilisateur(emailOrPhone: string, callback: (result: boolean) => void) {
    this._estimationService.getClients().subscribe(
      (clients: any) => {
        this.clients = clients.clients;
        console.log("Clients récupérés :", this.clients);
        console.log("Email ou téléphone fourni :", emailOrPhone);

        const client = this.clients.find((c: any) => {
          return (c.email?.trim().toLowerCase() === emailOrPhone.trim().toLowerCase() ||
            c.telephone?.replace(/\s/g, '') === emailOrPhone.replace(/\s/g, ''));
        });

        if (client) {
          console.log("Informations du client récupérées :", client);
          this.clientDetails = client;
          // Utiliser les informations du client dans la réservation
          this.reserverForm.patchValue({
            client_selectionne: client.id,
          });
          this.ajouterDonneesReservation();
          this.recupererDetailsVehicule();
          callback(true);
        } else {
          console.error("Aucun client trouvé avec cet email ou numéro de téléphone");
          callback(false);
        }
      },
      (error: any) => {
  ...
  validerNouveauClient(callback: (result: boolean) => void) {
    ...
    if (this.checkClientDataValidity(typeClient)) {
      ...
      console.error(
        "Les champs ne sont pas renseignés ou invalides pour le type de client: " +
        typeClient
      );
      callback(false);
    }
  ...
  private getClientsByEmailAndPrepareReservation(data: string, callback: () => void) {
    this.preRanplissage();
    // Préparation des données pour une nouvelle réservation
    const numeroReservationValue = this.genererNumeroReservation();
    this.reservationData = {
      numero_reservation: numeroReservationValue,
      datePriseEnCharge: this.reserverForm.get("datePriseEnCharge")?.value,
      dateRetour: this.reserverForm.get("dateRetour")?.value,
      coutTransport: this.reserverForm.get("coutTransport")?.value,
      coutMajorer: this.reserverForm.get("coutMajorer")?.value,
      lieuxPriseEnCharge: this.reserverForm.get("lieuxPriseEnCharge")?.value,
      lieuxDestination: this.reserverForm.get("lieuxDestination")?.value,
      distance: this.reserverForm.get("distance")?.value,
      duree: this.reserverForm.get("duree")?.value,
      typeReservation: this.reserverForm.get("typeReservation")?.value,
      vehicule: this.reserverForm.get("vehicule")?.value,
      distanceWaypoint: this.reserverForm.get("distanceWaypoint")?.value,
      dureeWaypoint: this.reserverForm.get("dureeWaypoint")?.value,
      destinationInputs: this.reserverForm.get("destinationInputs")?.value,
      attribut: this.reserverForm.get("attribut")?.value,
      coutTotalReservation: this.reserverForm.get("coutTotalReservation")?.value,
      totalAttributCost: this.reserverForm.get("totalAttributCost")?.value,
      note: this.reserverForm.get("note")?.value,
      nombreBagage: this.reserverForm.get("nombreBagage")?.value,
      nombrePassager: this.reserverForm.get("nombrePassager")?.value,
      compagnieAerienne: this.reserverForm.get("compagnieAerienne")?.value,
      modePaiement: this.reserverForm.get("modePaiement")?.value,
      statutReservation: this.reserverForm.get("statutReservation")?.value,
    };
    this.recupererDetailsVehicule();
  ...
  checkClientDataValidity(typeClient: string): boolean {
    if (
      typeClient === "client_simple" ||
      typeClient === "client_liee_agence" ||
      typeClient === "client_liee_societe"
    ) {
      return (
        this.isValid(this.reserverForm.get("first_name")) &&
      ...
      return (
        this.isValid(this.reserverForm.get("nom_societe")) &&
      ...
      return (
        this.isValid(this.reserverForm.get("nom_agence")) &&
